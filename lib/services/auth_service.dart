...
class AuthService {
  ...
  Future<AuthResult> connexion({
    ...
    try {
      ...
      // Se connecter anonymement juste pour la session Firebase
      if (_auth.currentUser == null) {
        await _auth.signInAnonymously()
            .timeout(const Duration(seconds: 10));
      }

      // Toujours utiliser le uid d'origine stocké dans Firestore
      final user = AppUser.fromMap(userData);
      return AuthResult.success(user);
    } on FirebaseAuthException catch (e) {
      return AuthResult.failure(_mapFirebaseError(e.code));
    } catch (e) {
      return AuthResult.failure('Une erreur est survenue. Vérifiez votre connexion.');
    }
  }
  ...
  Future<AuthResult> connexionParPin({required String pin}) async {
    try {
      ...
      // Se connecter anonymement si pas déjà connecté (juste pour la session)
      if (_auth.currentUser == null) {
        await _auth.signInAnonymously()
            .timeout(const Duration(seconds: 10));
      }

      // Toujours utiliser le uid stocké dans Firestore (celui de l'inscription)
      final user = AppUser.fromMap(userData);
      return AuthResult.success(user);
    } on FirebaseAuthException catch (e) {
      return AuthResult.failure(_mapFirebaseError(e.code));
